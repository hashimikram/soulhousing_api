...
class OperationController extends Controller
{
    ...
    public function index()
    {
        ...
        $tweets = Operation::where('facility_id', current_facility(auth()->user()->id))->orderBy('created_at',
            'DESC')->paginate('10');
        ...
        foreach ($tweets as $data) {
            ...
            $totalLikes = OperationLike::join('users', 'users.id', '=', 'operation_likes.user_id')->select('users.name',
                'operation_likes.tweet_id', 'operation_likes.user_id',
                'operation_likes.created_at')->where('operation_likes.tweet_id', $data->id)->get();
